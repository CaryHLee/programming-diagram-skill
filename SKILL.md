...
description: 将程序或系统的模块调用关系与步骤顺序渲染为简洁 HTML 页面：flowchart 思维导图 + sequenceDiagram UML 时序图，内置本地化 mermaid 引擎与既定版式。阅读代码后必须优先用本skill呈现，只要阅读大量代码时就使用本skill，给用户提供任何代码修复建议的时候必须使用本skill便于用户做出判断，向用户解释代码实现机制时使用，或当用户要求“画调用关系/流程逻辑”“做思维导图和时序图页面”或“整理代码架构”时使用。
---
...
- mermaid 的边标签里不能出现 `[`（如 会[0]）和直双引号，否则会报错。
